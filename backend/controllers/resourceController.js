...
exports.createResource = async (req, res) => {
  ...
  try {
    const [rRes] = await db.query(
      "INSERT INTO resource (Name,ResourceType,Description,OwnerId,IsActive) VALUES (?,?,?,?,1)",
      { transaction: t, replacements: [name, type, description || null, ownerId] }
    );
...
exports.listResources = async (req, res) => {
  try {
    ...
    const [rows] = await db.query(`SELECT * FROM resource ${where}`, { replacements: type ? [type] : [] });
    res.json(rows);
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
